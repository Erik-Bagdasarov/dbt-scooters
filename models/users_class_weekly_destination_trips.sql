with daily_stat_cte as (
{#
    For each users, we find the trip statistic to each destination for each day:
    mortning_trips - the number of morning trips
#}
        select
            user_id
        ,   st_snaptogrid(
        ...
        ,   date
        ,   count(case when extract(hour from started_at at time zone 'Europe/Moscow') between 6 and 10 then 1 end) as morning_trips
        from {{ ref('trips_prep') }}
        group by 1,2,3
    )
, weekly_stat_cte as (
...
#}
        ...
        ,   count(distinct
                  case when morning_trips > 0 then date end) as morning_trips_days
        from daily_stat_cte
        group by 1,2,3
    )
...
#}
        select
            user_id
        ,   destination
        ,   avg(morning_trips_days) as avg_morning_trips_days
        from weekly_stat_cte
        group by 1,2
    )
select
    user_id
,   max(avg_morning_trips_days) >= 3 as to_work
